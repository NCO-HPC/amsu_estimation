      ...
      subroutine amsua_ret_n15(nup,at,bt,ispot,ttype,bl,
     &                                tret,tpw,clq)
c
c    programmer  Mitch Goldberg  NOAA/NESDIS/ORA  301-763-8136
c                mgoldberg@nesdis.noaa.gov
c
c     07/09/98         Applies antenna correction and limb adjusts AMSU-A  
c     and compute retrievals
c    04/03/01        Modification for NOAA-15.  Note, I decided not to apply 
c     antenna corrections to NOAA-16 and NOAA-16, since I found that 
c     there is a larger difference between NOAA-16 and NOAA-15 when antenna 
c     corrections are used.  Better not to use antenna corrections.  I plan 
c     to regenerate coefficients for NOAA-15 w/o corrections.
c
c     input  integer    nup     regression coefficients, based 
c                               on simulated btemps, are used from
c                               level 1 to nup.  Typically nup us set to 10. 
...
c
      ...
      data delta/4*0.,-.36,-.94,-1.0,-.63,-.51,-.28,.22,.19,.24,0,0/ 
...
c
      OPEN(56,FILE='amsu_n15limb_sea.txt')
...
c
      OPEN(66,FILE='amsu_n15limb_land.txt')
...
c
      OPEN(59,FILE='amsu_n15regr_sea.txt')
      do i =9,40 ! levels
      read(59,*)
      read(59,*)
      ...
      close(59)
c
c     read in  non-sea temperature retrieval coefficients based on 
c     raob matchups for 5 to 1000 mb
c
      OPEN(59,FILE='amsu_n15regr_land.txt')
      do i =9,40 ! levels
      read(59,*)
      read(59,*)
...
c
      ...
      OPEN(59,FILE='amsu_n15amsusimreg.txt')
      do i =1,nup ! levels
      read(59,*)
      read(59,*)
      read(59,*) nchz(i), (nchanz(i,k),k=1,nchz(i))
...
c     read in temperature retrieval coefficients based on simulated 
c     rocketsondes 
c     same coefficients are read twice to allow for logic of 
