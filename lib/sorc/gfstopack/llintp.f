...
c
c Written: Jack Dostalek 2004 
c Modified: Jack Dostalek 1/30/2006
c   	Correction applied to prevent a potential problem when 
c	rlat is close to either pole
c
c
...
c        In order to correct a potential problem when rlat is close to
c        either pole, introduce variables arg0 arg1, and arg. In the 
c        subsequent calculations of dx0,dx1,and x, I'll use the fact 
c        that cosine is an even function to avoid if statements.
c        The problem occurred for situations when the spacing between 
c        points of grid 1 was greater than the spacing between points
c        of grid 2. When rlat was between the pole and the latitude of 
c        the first row equatorward of the pole in grid 1, dx0 would be
c        measured at the pole, so it is 0, or sometimes very small. The
c        variable x was not zero in these cases so the second term in
c        f2 became very large.  When rlat was at the pole, the problem 
c	 did not occur because even though dx0 was still small, x=0, so
c	 the second term of f2 did not contribute.  This was found when
c   	 the NCEP/NCAR reanalysis (2.5 x 2.5) was interpolated to a global
c	 2.0 x 2.0 grid. 
